...
export default function DrmMovieController() {
    ...
    const [pallyConContentConfigs, setPallyConContentConfigs] = useState<PallyConContentConfiguration[]>([]);
    ...
    const setPallyConEvents = () => {
        if (listeners.length == 0) {
            const events = [];
            events.push(PallyConMultiDrmSdk.addPallyConEvent(PallyConEventType.complete, (event) => {
                updateMovies(event.url, 'downloadState', downloadState.success)
            }));

            events.push(PallyConMultiDrmSdk.addPallyConEvent(PallyConEventType.pause, (event) => {
                updateMovies(event.url, 'downloadState', downloadState.pause)
            }));

            events.push(PallyConMultiDrmSdk.addPallyConEvent(PallyConEventType.remove, (event) => {
                updateMovies(event.url, 'downloadState', '')
            }));

            events.push(PallyConMultiDrmSdk.addPallyConEvent(PallyConEventType.stop, (event) => {
                updateMovies(event.url, 'downloadState', '')
            }));

            events.push(PallyConMultiDrmSdk.addPallyConEvent(PallyConEventType.download, (event) => {
                updateMovies(event.url, 'downloadState', downloadState.running)
            }));

            events.push(PallyConMultiDrmSdk.addPallyConEvent(PallyConEventType.contentDataError, (event) => {
                setError(event.errorCode + ': ' + event.message);
            }));

            events.push(PallyConMultiDrmSdk.addPallyConEvent(PallyConEventType.drmError, (event) => {
                setError(event.errorCode + ': ' + event.message);
                updateMovies(event.url, 'downloadState', downloadState.failed)
            }));

            events.push(PallyConMultiDrmSdk.addPallyConEvent(PallyConEventType.licenseServerError, (event) => {
                setError(event.errorCode + ': ' + event.message);
                updateMovies(event.url, 'downloadState', downloadState.failed)
            }));

            events.push(PallyConMultiDrmSdk.addPallyConEvent(PallyConEventType.downloadError, (event) => {
                setError(event.errorCode + ': ' + event.message);
                updateMovies(event.url, 'downloadState', downloadState.failed)
            }));

            events.push(PallyConMultiDrmSdk.addPallyConEvent(PallyConEventType.networkConnectedError, (event) => {
                setError(event.errorCode + ': ' + event.message);
                updateMovies(event.url, 'downloadState', downloadState.failed)
            }));

            events.push(PallyConMultiDrmSdk.addPallyConEvent(PallyConEventType.detectedDeviceTimeModifiedError, (event) => {
                setError(event.errorCode + ': ' + event.message);
                updateMovies(event.url, 'downloadState', downloadState.failed)
            }));

            events.push(PallyConMultiDrmSdk.addPallyConEvent(PallyConEventType.migrationError, (event) => {
                setError(event.errorCode + ': ' + event.message);
                updateMovies(event.url, 'downloadState', downloadState.failed)
            }));

            events.push(PallyConMultiDrmSdk.addPallyConEvent(PallyConEventType.licenseCipherError, (event) => {
                setError(event.errorCode + ': ' + event.message);
            }));

            events.push(PallyConMultiDrmSdk.addPallyConEvent(PallyConEventType.unknownError, (event) => {
                setError(event.errorCode + ': ' + event.message);
                updateMovies(event.url, 'downloadState', downloadState.failed)
            }));

            events.push(PallyConMultiDrmSdk.addPallyConEvent(PallyConEventType.progress, (event) => {
    ...
    const updateMovies = useCallback((url: string | null, name: any, value: any) => {
    ...
    const setConfigs = async (drmMovies: DrmMovie[]) => {
        ...
        for (let i = 0; i < drmMovies.length; i++) {
            ...
            const needsMigration =  await PallyConMultiDrmSdk.needsMigrateDatabase(config);
            if (needsMigration) {
    ...
    const getPlayerData = async (movie: DrmMovie): Promise<string> => {
        ...
        const index = pallyConContentConfigs.findIndex((element) =>
            element.contentUrl === movie.url);

        try {
            return await PallyConMultiDrmSdk.getObjectForContent(pallyConContentConfigs[index]);
        } catch (e: any) {
            setError(e.message);
            return '';
        }
    ...
    const downloadMovie = async (movie: DrmMovie): Promise<void> => {
        ...
        if (movies[index].downloadState === downloadState.pause) {
            ...
            try {
                await PallyConMultiDrmSdk.addStartDownload(pallyConContentConfigs[index]);
            } catch (e: any) {
                setError(e.message);
            }
        }
    ...
    const updateProgress = (url: string | null, percent: number | null) => {
        const index = downloadPercent.findIndex((element) =>
            element[0] === url)
        if (url !== null && index < 0) {
